...
pub(crate) static VALIDATION_KEYWORDS: &[&str] = &[
    "range", "regex", "contains", "length", "required",
];
...
impl ValidationAttrs {
    pub fn new(attrs: &[syn::Attribute], errors: &Ctxt) -> Self {
        ValidationAttrs::default()
            .populate(attrs, "fog", false, errors)
    }
    ...
    fn populate(
        ...
        let mutual_exclusive_error = |path: &Path, other: &str| {
            if !ignore_errors {
                let msg = format!(
                    "fog attribute cannot contain both `{}` and `{}`",
                    path.get_ident().unwrap(),
                    other,
                );
                errors.error_spanned_by(path, msg)
        ...
        let duplicate_format_error = |existing: Format, new: Format, path: &syn::Path| {
            if !ignore_errors {
                let msg = if existing == new {
                    format!("duplicate fog attribute `{}`", existing.attr_str())
                } else {
                    format!(
                        "fog attribute cannot contain both `{}` and `{}`",
                        existing.attr_str(),
                            ...
                            meta => {
                                if !ignore_errors {
                                    errors.error_spanned_by(
                                        meta,
                                        "unknown item in fog length attribute".to_string(),
                                    );
                                }
                            }
                        }
                        ...
                        match nested {
                            ...
                            meta => {
                                if !ignore_errors {
                                    errors.error_spanned_by(
                                        meta,
                                        "unknown item in fog range attribute".to_string(),
                                    );
                                }
                            }
                        }
                                ...
                                match x {
                                    ...
                                    meta => {
                                        if !ignore_errors {
                                            errors.error_spanned_by(
                                                meta,
                                                "unknown item in fog regex attribute".to_string(),
                                            );
                                        }
                                    }
                                }
                                ...
                                match x {
                                    ...
                                    meta => {
                                        if !ignore_errors {
                                            errors.error_spanned_by(
                                                meta,
                                                "unknown item in fog contains attribute".to_string(),
                                            );
                                        }
                                    }
                                }
